...
ALL_SEARCHES = {
    ...
    "Is your card historic?": ["is:historic", "Your card is an artifact, legendary creature, or saga enchantment."],
    ...
    # "Is your card have color identity red?": "id:r -id:c",
    # "Is your card have color identity blue?": "id:u -id:c",
    # "Is your card have color identity green?": "id:g -id:c",
    # "Is your card have color identity white?": "id:w -id:c",
    # "Is your card have color identity black?": "id:b -id:c",
    # "Is your card have color identity colorless?": "id:c",
    # Mana Cost QUERIES
    ...
    # Oracletext QUERIES
    "Can your card become unblockable to certain creatures?": ["o:'~ can' o:'t be blocked'"],
    ...
    "Does your card have a triggered ability?": ["o:when or o:'at the'", "Your card contains the word 'when', 'whenever' or 'at' in its effect text."],
    ...
    "Does your card have an enters the battlefield trigger for permanents you control?": ["(o:enters o:the o:battlefield) and -(o:'may have ~') and (o:under o:your o:control)"],
    ...
    "Does your card have an ability that targets players?": ["o:target o:' player'"],
    ...
    "Does your card care about monarchs?": ["o:monarch"],
...
def generate_questions_scryfall_catalog():
    BASE_SCRYFALL_CATALOG_URL = "https://api.scryfall.com/catalog/"
    CATALOG_TYPES = {
        "creature-types": [],
        "planeswalker-types": [],
        "land-types": [],
    ...
    # For each catalog type, we'll gather the range of values
    total = 0
    for catalog_type in CATALOG_TYPES:
        url = BASE_SCRYFALL_CATALOG_URL + catalog_type
        req = s.get(url)
        valueslist = []
        req_js = req.json()
        data = req_js.get("data")
        if not data:
            print(catalog_type)
            print("Failed to make a page request when getting types from " + url)
        for value in data:
            valueslist.append(value.lower())
        CATALOG_TYPES[catalog_type] = valueslist
        total += len(CATALOG_TYPES[catalog_type])

    print("Finished gathering all values for each catalog type.")
    print("Generating queries for scryfall per value per catalog type.")
    print(f"Expecting to generate {total} new questions.")

    CATALOG_TO_QUERY_MAP = {
        "creature-types": "t:@",
        "planeswalker-types": "t:@",
        "land-types": "t:@",
        "artifact-types": "t:@",
        "enchantment-types": "t:@",
        "spell-types": "t:@",
        ...
        "toughnesses": "tou:@",
        "loyalties": "loyalty:@",
        "keyword-abilities": "keyword:'@' or o:'@'",
        "ability-words": "keyword:'@' or o:'@'",
        "supertypes": "t:@",
    }

    CATALOG_TO_QUESTION_PREFIX_MAP = {
        "creature-types": "Is your card a creature with type @",
        "planeswalker-types": "Is your card the planeswalker @",
        "land-types": "Is your card a land with type @",
        "artifact-types": "Is your card an artifact with type @",
        "enchantment-types": "Is your card an enchantment with type @",
        "spell-types": "Is your card a @",
        "powers": "Is your card a creature with power @",
        "toughnesses": "Is your card a creature with toughness @",
        "loyalties": "Is your card a planeswalker with a starting loyalty of @",
        "keyword-abilities": "Does your card have the keyword @",
        "ability-words": "Does your card have the keyword @",
        "supertypes": "Is your card a @",
    }

    SCRYFALL_QUESTIONS_LIST = {}

    for catalog in CATALOG_TO_QUERY_MAP:
        all_values = CATALOG_TYPES[catalog]
        query_template = CATALOG_TO_QUERY_MAP[catalog]
        for value in all_values:
            query_str = query_template.replace("@", str(value))
            question = CATALOG_TO_QUESTION_PREFIX_MAP[catalog].replace("@", str(value))
            SCRYFALL_QUESTIONS_LIST[question] = [query_str]
    return SCRYFALL_QUESTIONS_LIST
...
def make_page_request(URL, question=""):
    ...
    if not data:
        print(question)
        print(
            "Failed to make a page request when asking "
            + question
...
def ask_all_questions():
    questions_cards_map = {}
    ALL_SEARCHES.update(generate_questions_scryfall_catalog())
    index = 0
    for question_text, question in ALL_SEARCHES.items():
        cards_returned = ask_question(question[0])
        # print("Answered with " + str(len(cards_returned)) + " cards returned.")
        questions_cards_map[question_text] = cards_returned
        index += 1
        if index % 50 == 0:
            print(f"Asked up to {index} questions, so far")
    return questions_cards_map

def get_all_cardnames():
    CARD_NAME_CATALOG_URL = "https://api.scryfall.com/catalog/card-names"
    res = s.get(CARD_NAME_CATALOG_URL)
    res_js = res.json()
    data = res_js.get("data")
    if not data:
        print("Failed to make page request when getting all card names.")
    return data
### UPDATES ALL SEARCHES DEFINE AT TOP OF PAGE WITH SCRYFALL CATALOG QUESTIONS
# ALL_SEARCHES.update(generate_questions_scryfall_catalog())
